...
def arc_limits_mask(_ds, _var, _faces):
    ...
    *nnn, DS = arct_connect(_ds, _var, faces=_faces, masking=True, opt=False)  ## This only works in the case the transformation involves the whole domain
    ...
    if type(DSa2) != _dstype:
        ...
        [Xi_2, Xf_2] = [int(DSa2[_var].X[0]), edge_arc_data(DSa2[_var], 2, dims)]
    if type(DSa5) != _dstype:
        DSa5 = 0
        [Yi_5, Yf_5] = [0, 0]
    else:
        [Yi_5, Yf_5] = [int(DSa5[_var].Y[0]), edge_arc_data(DSa5[_var], 5, dims)]
    if type(DSa7) != _dstype:
        DSa7 = 0
        [Xi_7, Xf_7] = [0, 0]
    else:
        [Xi_7, Xf_7] = [edge_arc_data(DSa7[_var], 7, dims), int(DSa7[_var].X[-1])]
    if type(DSa10) != _dstype:
        DSa10 = 0
        [Yi_10, Yf_10] = [0, 0]
    else:
        [Yi_10, Yf_10]= [edge_arc_data(DSa10[_var], 10, dims), int(DSa10[_var].Y[-1])]
