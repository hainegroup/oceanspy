...
# Test oceandataset
od4calc = open_oceandataset.from_netcdf("{}MITgcm_rect_nc.nc" "".format(Datadir))
ds = od4calc.dataset
step = 1.0e-2
...
sintime = xr.zeros_like(od4calc.dataset["Temp"]) + np.sin(
    (od4calc.dataset["time"] - od4calc.dataset["time"][0]) / np.timedelta64(1, "s")
)
...
def test_divergence(od, varNameList):
    ...
    # Compute divergence
    dive_ds = divergence(
        od, iName=varNameList[0], jName=varNameList[1], kName=varNameList[2]
    )
...
def test_integral(od, varNameList):
    ...
    var = int_ds[int_name]
    check = (
        w_mean_ds["w_mean_" + varNameList] * w_mean_ds["weight_" + varNameList].sum()
    )
...
def test_int_mean_options(od, varNameList, axesList, storeWeights):
    if axesList == "wrong":
        ...
        weighted_mean(
            od, varNameList=varNameList, axesList=axesList, storeWeights=storeWeights
        )
