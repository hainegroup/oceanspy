...
from oceanspy import open_oceandataset, OceanDataset, utils
...
# Create an oceandataset for testing calculus functions
od_curv = open_oceandataset.from_netcdf('./oceanspy/tests/Data/MITgcm_curv_nc.nc')
...
def test_potential_density_anomaly(od_in, eq_state):
    ...
    # Check values
    Sigma0 = eval("utils.dens{}(od_in._ds['S'].values, od_in._ds['Temp'].values, 0)".format(od_in.parameters['eq_state']))
...
def test_vertical_relative_vorticity(od_in):
    ...
    ds = od.dataset
    if vortName is None: ds = ds.drop('momVort3')
    else:                ds = ds.drop(vortName)
    od_in = OceanDataset(ds).set_aliases(aliases)
...
def test_mooring_horizontal_volume_transport(od_in, mooring, closed, flippedX, flippedY):
    
    if mooring is True:
        if not closed:
            od_moor = od_in.subsample.mooring_array(Xmoor=[od_in.dataset['X'].min().values,
                                                           od_in.dataset['X'].max().values],
                                                    Ymoor=[od_in.dataset['Y'].min().values,
                                                           od_in.dataset['Y'].max().values])
            if flippedX and not flippedY:
                od_moor = od_in.subsample.mooring_array(Xmoor=[od_in.dataset['X'].max().values,
                ...
                od_moor = od_in.subsample.mooring_array(Xmoor=[od_in.dataset['X'].max().values,
                                                               od_in.dataset['X'].min().values],
                                                        Ymoor=[od_in.dataset['Y'].max().values,
...
def test_survey_aligned_velocities(od_in, gridtype, rotate, survey):
    ...
    if survey is True:
        od_surv = od_in.subsample.survey_stations(Xsurv=[od_in.dataset['XC'].min().values,
                                                         od_in.dataset['XC'].max().values],
                                                  Ysurv=[od_in.dataset['YC'].min().values,
                                                         od_in.dataset['YC'].max().values],
                                                  delta=2)
        # Align
        if gridtype is 'rect' or rotate is False:
        ...
        if rotate is False:
            ...
            assert_almost_equal(vel_surv.values, vel_alig.values, 4) #<-------------- Is this right? Check with Renske!
...
def test_salt_budget(od_in):
    ...
    # Mask and check
    mask  = xr.where(np.logical_or(check.isnull(), var.isnull()), 0, 1)
    assert_allclose(check.where(mask, drop=True).values, var.where(mask, drop=True).values, 1.E-6, equal_nan=False) #<------ Not sure why S it's a little less accurate
