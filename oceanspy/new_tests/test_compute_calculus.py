...
from oceanspy.compute import gradient, divergence, curl, laplacian, weighted_mean, integral

# Create an oceandataset for testing calculus functions
od4calc  = open_oceandataset.from_netcdf('./oceanspy/tests/Data/MITgcm_rect_nc.nc')
ds = od4calc.dataset
step = 1.E-2
...
for i in range(len(ds['time_midp'])):
    T = T + [np.datetime64(t0) + np.timedelta64(int(i*step*1.E3), 'ms')]
ds['time_midp'] = np.array(T,dtype='datetime64') + np.timedelta64(int(0.5*step*1.E3), 'ms')
...
sintime = (xr.zeros_like(od4calc.dataset['Temp']) +  
           np.sin((od4calc.dataset['time']-od4calc.dataset['time'][0])/np.timedelta64(1, 's')))
...
costime = (xr.zeros_like(od4calc.dataset['oceSPtnd'])+ 
           np.cos((od4calc.dataset['time_midp']-od4calc.dataset['time_midp'][0])/np.timedelta64(1, 's')))
...
sin_ds  = xr.Dataset({'sinX': sinX, 'sinY': sinY, 'sinZ': sinZ, 'sintime': sintime,
                      'cosX': cosX, 'cosY': cosY, 'cosZ': cosZ, 'costime': costime,
...
def test_all_gradients(od):
    ...
    with pytest.warns(UserWarning):
        od_surv = od.subsample.survey_stations(Xsurv=od_moor.dataset['XC'].squeeze().values, 
                                               Ysurv=od_moor.dataset['YC'].squeeze().values)
    # Test all dimension
    DIMS = []
    VARS = []
    for var in od.dataset.data_vars:
        ...
        append = True
        for dims in DIMS:
            if set(this_dims).issubset(set(dims)) and set(dims).issubset(set(this_dims)): 
                append = False
                continue
        if append:
            VARS = VARS + [var]
...
# DIVERGENCE
@pytest.mark.parametrize("od, iName, jName, kName", [(od4calc, None, None, 'Temp'),
...
def test_divergence(od, varNameList):
    ...
    # Add units
    if not None in varNameList:
        for varName in varNameList:
            od._ds[varName].attrs['units'] = 'm/s'
        
    # Compute divergence
    dive_ds = divergence(od, iName=varNameList[0], jName=varNameList[1], kName=varNameList[2]) 
    ...
    for varName in varNameList:
        if varName is not None:
            ...
            # Assert using numpy
            assert_array_almost_equal(var.where(mask, drop=True).values,
                                      check.where(mask, drop=True).values, 1.E-3)

        
# CURL
@pytest.mark.parametrize("od, iName, jName, kName", [(od4calc, None, None, None),
...
def test_laplacian(od, varNameList, axesList):
    if varNameList=='U' or axesList=='wrong':
        ...
        lap_ds = laplacian(od, varNameList=varNameList, axesList=axesList)
...
def test_integral(od, varNameList):
    ...
    var = int_ds[int_name]
    check = (w_mean_ds['w_mean_'+varNameList]*w_mean_ds['weight_'+varNameList].sum())
